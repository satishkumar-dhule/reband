...
**Status:** ✅ NO AUTHENTICATION (Static-first architecture)

**All routes are publicly accessible (no server-side auth):**
...
- `POST /api/user/sessions` - Creates sessions (stored locally in browser)
- `POST /api/history` - Creates history (stored locally in browser)
- `POST /api/learning-paths/:pathId/start` - Modifies data (public aggregate count)
- `DELETE /api/user/sessions/:sessionId` - Modifies data (stored locally in browser)
