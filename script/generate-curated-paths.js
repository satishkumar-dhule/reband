...
async function main() {
  ...
  // Minified JSON for production (no whitespace)
  const minified = JSON.stringify(exportPaths);
  fs.writeFileSync(outputPath, minified);
  console.log(`   ✅ Exported ${exportPaths.length} paths to ${outputPath} (${formatBytes(minified.length)})`);
}
