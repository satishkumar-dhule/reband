...
- Security & Privacy: No secrets in frontend; no server-side user authentication; data exposed is sanitized; dependencies patched.
...
- No server-side user authentication or accounts; all user data stored locally in browser
...
## UI/UX Standards (from UNIFIED_CONTROLS_SPEC.md)

### Control Components

| Component | Import |
|-----------|--------|
| Button | `@/components/unified/Button` |
...
| `--gh-accent-emphasis` | Accent color |
| `--gh-focus-ring` | Focus ring |

### Prohibited Patterns
- ❌ `bg-blue-500`, `px-5`, custom padding
- ❌ Custom button styles, hardcoded border colors
- ✅ GitHub CSS variables, Tailwind spacing, unified components

### Accessibility Requirements
- Keyboard-accessible focus with visible focus rings
- Form controls associated with labels via `htmlFor`/`id`
