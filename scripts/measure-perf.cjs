// Perf measurement tool using Playwright (CommonJS)
'use strict';
const fs = require('fs');
...
async function measureForUrl(url, dir) {
  ...
  try {
    ...
    // Take screenshot with a retry to handle transient protocol errors
    try {
      await page.screenshot({ path: screenshotPath, fullPage: true });
    } catch (ssErr) {
      console.warn(`Screenshot failed for ${url}, retrying once...`, ssErr);
      try {
        await page.screenshot({ path: screenshotPath, fullPage: true });
      } catch (ssErr2) {
        console.warn(`Retry failed for ${url} screenshot`, ssErr2);
      }
    }
  } catch (err) {
...
async function main() {
  ...
  const urls = urlsArg.split('=')[1] ? urlsArg.split('=')[1].split(',') : [
    ...
    'https://open-interview.github.io/login',
    'https://open-interview.github.io/channel/algorithms',
