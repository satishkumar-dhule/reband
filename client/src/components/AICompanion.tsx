...
export function AICompanion({ pageContent, onNavigate, onAction, availableActions = [] }: AICompanionProps) {
  ...
  // Initialize speech recognition using PTT hook
  const {
    transcript: pttTranscript,
    start: startPTT,
    stop: stopPTT,
    reset: resetPTT,
    isListening: pttIsListening,
    isSupported: pttSupported
  } = useSpeechRecognitionPTT({
    interimResults: true,
    lang: voiceLang,
    onResult: (result) => {
      if (result.isFinal) {
    ...
    onEnd: () => {
      ...
      if (voiceMode && pttTranscript.trim().length > 0) {
        console.log('PTT released, auto-sending:', pttTranscript);
        setTimeout(() => {
          sendMessage();
        }, 300);
      }
  ...
  // Settings storage key - using sessionStorage for security
...
// Security: Using sessionStorage instead of localStorage for API key storage
  ...
  const startListening = () => {
    if (pttSupported && !pttIsListening) {
      try {
        resetPTT();
        startPTT();
