...
// Initialize Google Analytics (non-blocking, performance-optimized)
export function initializeAnalytics(measurementId: string) {
  ...
  // Preconnect to Google domains for faster script loading
  ...
  // Initialize dataLayer BEFORE loading the script (prevents race conditions)
  ...
  gtag('config', measurementId, {
    ...
    // Performance: reduce GA payload by disabling unused features
    'send_page_view': true,
    'transport_type': 'beacon', // Use Beacon API for better performance
  });

  // Load GA script with low priority to not block rendering
  const script = document.createElement('script');
  script.async = true;
  script.defer = true; // Defer ensures it doesn't block parsing
  script.src = `https://www.googletagmanager.com/gtag/js?id=${measurementId}`;
  // Set fetchpriority to low so browser prioritizes critical resources
