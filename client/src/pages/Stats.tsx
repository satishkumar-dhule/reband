...
import {
  AppLayout, SEOHead, StatsSkeleton,
  Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbPage, BreadcrumbSeparator,
  PageContainer, PageHeader, SectionHeader, GhStatCard,
} from '@/lib/ui';
...
import {
  Trophy, Flame, Zap, Target, Home, CheckCircle2, Clock, BarChart2,
    ...
    <>
      ...
      <AppLayout>
        <PageContainer maxWidth="5xl" id="main-content">
          ...
          <PageHeader title="Progress Stats" />

          {/* Stats overview */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <GhStatCard
              data-testid="stat-total-completed"
              label="Total Completed"
              value={totalCompleted}
              valueSuffix={`/ ${totalQuestions}`}
            ...
            <GhStatCard
              ...
              value={streak}
              valueSuffix="days"
              icon={<Flame className="w-4 h-4 text-[var(--gh-attention-fg)]" />}
            />
            <GhStatCard
              data-testid="stat-xp"
              label="Total XP"
              value={balance.toLocaleString()}
              icon={<Zap className="w-4 h-4 text-[var(--gh-accent-fg)]" />}
            />
            <GhStatCard
              data-testid="stat-level"
              label="Level"
              value={level}
              icon={<Trophy className="w-4 h-4 text-[var(--gh-attention-fg)]" />}
            />
          </div>
                      ...
                      <tr key={mod.id} className="hover:bg-[var(--gh-canvas-subtle)] transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <ChannelIcon className="w-5 h-5 text-[var(--gh-fg-muted)]" />
