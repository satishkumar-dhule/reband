...
export default function VoiceSession() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        // Load questions for subscribed channels (or defaults if none subscribed)
        const channelIds = preferences.subscribedChannels.length > 0
          ? preferences.subscribedChannels
          : ['system-design', 'algorithms', 'frontend', 'backend', 'behavioral'];

        const results = await Promise.all(
          channelIds.map(ch => loadChannelQuestions(ch).catch(() => [] as Question[]))
        );
        ...
        const filteredSessions = sessions.filter(s => channelIds.includes(s.channel));
