...
In the frontend, all data fetching must use this pattern (NOT Express API in production):
...
2. `client/src/lib/api.ts` — smart data fetcher (static mode vs dev mode)
3. `vite.config.ts` — ensure `base` uses `VITE_BASE_URL` env var (already done)
4. `.github/workflows/deploy-static.yml` — CI/CD pipeline
5. `public/.nojekyll` — required by GitHub Pages to serve Vite assets correctly
